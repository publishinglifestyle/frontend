...
export const getTranslations = async (detectedLanguage: string): Promise<Translations> => {
    ...
    try {
        ...
        const translations: Translations = await import(`../translations/${language}.json`).then(module => module.default);
        return translations;
    } catch (error) {
        console.error('Error loading translation file:', error);
        // Fallback to default language if there's an error and cast to Translations
        const translations: Translations = await import(`../translations/${defaultLanguage}.json`).then(module => module.default);
        return translations;
    }
};
