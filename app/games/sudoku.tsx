  ...
  const generatePDF = (sudokuData: any[]) => {
    ...
    sudokuData.forEach((sudoku, index) => {
      ...
      const puzzleTitle = is_sequential
        ? `Puzzle ${index + 1}`
        : custom_name || `Custom Puzzle ${index + 1}`;
      const solutionTitle = is_sequential
        ? `Solution ${index + 1}`
        : custom_solution_name || `Custom Solution ${index + 1}`;
      ...
      for (let page = 0; page < solutionsPages; page++) {
        ...
        doc.text(
          is_sequential
            ...
            : custom_solution_name || "Solution",
          pageWidth / 2,
          margin,
          { align: "center" }
        );

        const solutionsToShow = sudokuData.slice(
        ...
        solutionsToShow.forEach((sudoku, index) => {
          const offsetX =
            (index % gridPerRow) * (gridSize + 20) +
            (pageWidth - gridPerRow * gridSize - (gridPerRow - 1) * 20) / 2;
          const offsetY =
    ...
    for (let i = 0; i < 81; i++) {
      ...
      if (value !== "-") {
        doc.text(
          ...
          offsetY + row * cellSize + cellSize / 2 + 3,
          {
            align: "center",
          }
        );
