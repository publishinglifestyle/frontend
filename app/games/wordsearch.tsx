...
export default function WordSearch({
  ...
  const handleGenerateImages = async () => {
    ...
    try {
      console.log(`Generating Images with Font: ${font}, GridFontSize: ${fontSize}, GridSize: ${grid_size}, SolutionsPerPage: ${solutions_per_page}`);
      ...
      const effectiveSolsPerPage = solutions_per_page > 0 ? solutions_per_page : wordSearchResponses.length; // Ensure positive
      const totalSolutionPages = Math.ceil(wordSearchResponses.length / effectiveSolsPerPage);
      ...
      for (let page = 0; page < totalSolutionPages; page++) {
        ...
        await generateMultiSolutionImagePage(solutionChunk, page, startIndex, wordSearchResponses.length);
      }
  ...
  const generateSinglePageImage = async (wordSearchData: any, index: number, isSolution: boolean): Promise<void> => {
    return new Promise((resolve) => {
      const { grid, words: solutionWords } = wordSearchData;
      if (!grid || grid.length === 0) {
        console.warn(`Skipping single image generation for invalid data at index ${index}`);
        resolve(); return;
      }
      ...
      if (!ctx) { console.error("Failed to get 2D context"); resolve(); return; }
      ...
      const maxGridHeightAllowed = availableHeight - (currentY - margin) - wordListEstimateHeight - (isSolution ? 0 : 20);
      ...
      // Draw Grid
      console.log(`Drawing ${isSolution ? 'Solution' : 'Puzzle'} ${index + 1} onto Single Canvas with: cellPx=${gridCellSizePx.toFixed(1)}, fontSize=${fontSize}, font=${font}`);
      ...
      if (!isSolution) {
        ...
        const wordsPerLine = 5; const columnSpacing = 30;
        ...
        const wordListStartX = margin + (availableWidth - wordListTotalWidth) / 2;
        const sortedWords = [...solutionWords].sort((a, b) => a.clean.localeCompare(b.clean));
        sortedWords.forEach((wordData, i) => {
          const colIndex = i % wordsPerLine; const rowIndex = Math.floor(i / wordsPerLine);
          ...
          if (wordY < imageHeight - margin) {
            ctx.fillText(wordData.clean.toUpperCase(), wordX, wordY, listColumnWidth);
          } else if (rowIndex === Math.floor(i / wordsPerLine) && colIndex === 0) {
  ...
  const generateMultiSolutionImagePage = async (
    ...
    return new Promise((resolve) => {
      ...
      canvas.width = imageWidth; canvas.height = imageHeight;
      const ctx = canvas.getContext("2d");
      if (!ctx) { console.error("Failed to get 2D context for multi-solution page"); resolve(); return; }
      ...
      const endNum = Math.min(globalStartIndex + solutionChunk.length, totalPuzzles);
      const titleText = is_sequential
        ? `Solutions ${startNum} - ${endNum}`
        : custom_solution_name || `Word Search Solutions (Page ${pageIndex + 1})`;
      ctx.fillStyle = "black"; ctx.textAlign = "center"; ctx.textBaseline = "top";
      ctx.font = `bold ${solutionTitleFontSize}px "${font}", ${font}, Helvetica, Arial, sans-serif`;
      const titleY = margin;
      ctx.fillText(titleText, imageWidth / 2, titleY);
      let currentContentY = titleY + solutionTitleFontSize * lineSpacing + 20; // Starting Y for grids

      // --- Calculate Grid Layout ---
      const numGridsOnPage = solutionChunk.length;
      const gridsPerRow = Math.max(1, Math.floor(Math.sqrt(numGridsOnPage))); // Aim for squarish
      const gridsPerCol = Math.ceil(numGridsOnPage / gridsPerRow);
      const availableGridAreaHeight = availableHeight - (currentContentY - margin);
      const slotWidthPx = Math.floor((availableWidth - (gridsPerRow - 1) * solutionGridSpacing) / gridsPerRow);
      const slotHeightPx = Math.floor((availableGridAreaHeight - (gridsPerCol - 1) * solutionGridSpacing) / gridsPerCol);

      if (slotWidthPx <= 0 || slotHeightPx <= 0) {
        console.error("Calculated slot dimensions are invalid for multi-solution layout.");
        resolve(); return;
      }
      ...
      solutionChunk.forEach((wordSearch, chunkIndex) => {
        ...
        const slotOffsetX = margin + colIndex * (slotWidthPx + solutionGridSpacing);
        const slotOffsetY = currentContentY + rowIndex * (slotHeightPx + solutionGridSpacing);

        // Calculate cell size and final grid size to fit the slot
        let gridCellSizePx = Math.min(slotWidthPx / actualGridSize, slotHeightPx / actualGridSize);
        ...
        const solutionGridFontSize = Math.max(8, fontSize * (gridCellSizePx / 25)); // Example scaling, adjust base (25px?) or make fixed

        console.log(`  Drawing solution ${globalStartIndex + chunkIndex + 1} onto multi-solution canvas: cellPx=${gridCellSizePx.toFixed(1)}, fontSize=${solutionGridFontSize.toFixed(1)}`);
        drawWordSearchGridOnCanvas({
      ...
      try {
        const dataUrl = canvas.toDataURL("image/png");
        const baseFilename = custom_solution_name ? custom_solution_name.replace(/[^a-z0-9]/gi, '_').toLowerCase() : 'wordsearch_solutions';
        const filename = `${baseFilename}_page_${pageIndex + 1}.png`;
        downloadDataUrl(dataUrl, filename);
        console.log(`Multi-solution image generated: ${filename}`);
      } catch (e) {
        console.error("Error converting multi-solution canvas or downloading:", e);
        alert(`Failed to save solution image page ${pageIndex + 1}: ${e.message}`);
      } finally {
        resolve();
      }
  ...
  // ==================================================================
  const drawWordSearchGridOnCanvas = ({ /* ... params ... */
    ctx, grid, offsetX, offsetY, cellSizePx, showWords, words, fontSizePx, fontFamily,
  }: {
    ctx: CanvasRenderingContext2D; grid: Cell[][]; offsetX: number; offsetY: number; cellSizePx: number;
    showWords: boolean; words?: Word[]; fontSizePx: number; fontFamily: string;
  }) => {
    ...
    for (let r = 0; r < gridSize; r++) {
      for (let c = 0; c < gridSize; c++) {
        const cell = grid[r]?.[c]; const letter = cell?.letter;
          ...
          word.path.forEach((coord) => {
            if (coord && typeof coord.x === 'number' && coord.x >= 0 && coord.x < gridSize && typeof coord.y === 'number' && coord.y >= 0 && coord.y < gridSize) {
              ...
              if (!highlightedCells.has(cellKey)) {
                ...
                const x = coord.x * cellSizePx; const y = coord.y * cellSizePx;
