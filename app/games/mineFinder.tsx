...
export default function MineFinder({ width = 10, height = 10, mines, font, num_puzzles = 1, solutions_per_page = 1, custom_name, custom_solution_name }: MineFinderProps) {
    ...
    const generatePDF = (minefields: any[]) => {
        ...
        minefields.forEach((minefieldData, index) => {
            ...
            doc.setFontSize(20);
            doc.text(custom_name || `Minesweeper Puzzle ${index + 1}`, pageWidth / 2, margin, { align: 'center' });
            ...
            if (num_puzzles === 1 || solutions_per_page === 1) {
                ...
                doc.addPage();
                doc.text(`Minesweeper Solution ${index + 1}`, pageWidth / 2, margin, { align: 'center' });
        ...
        if (num_puzzles > 1) {
            ...
            for (let page = 0; page < solutionsPages; page++) {
                ...
                doc.setFont(font || "times", "normal");
                doc.setFontSize(20);
                doc.text(custom_solution_name || `Solutions ${page * solutions_per_page + 1} - ${Math.min((page + 1) * solutions_per_page, num_puzzles)}`, pageWidth / 2, margin, { align: 'center' });
                ...
                solutionsToShow.forEach((minefield, index) => {
                    ...
                    const offsetY = margin + 30 + Math.floor(index / gridPerRow) * (gridSize + 20);
                    drawMinefieldGrid(doc, minefield.solution, offsetX, offsetY, adjustedCellSize, true);  // Solution with bombs
                });
            }
        }
        ...
        gridData.forEach((row: any[]) => {
            row.forEach((cell: any) => {
                ...
                if (showBombs && cell.isMine) {
                    ...
                    doc.text(cell.mines.toString(), x + cellSize / 2, y + cellSize / 2 + numberFontSize / 3, { align: 'center' });
                }
            });
        });
    };
