...
const PictureGenerationModal: React.FC<PictureGenerationModalProps> = ({ isOpen, onClose, onConfirm }) => {
        ...
        <Modal isOpen={isOpen} onClose={onClose} size="sm" isDismissable={false} isKeyboardDismissDisabled={true}>
            <ModalContent>
                <ModalHeader className="modal-header justify-center">
                    <h1 style={{ fontSize: "20px" }}>{translations?.selectNumberOfPictures}</h1>
                </ModalHeader>
                <ModalBody style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', textAlign: "center", flexDirection: 'column' }}>
                    <Select
                        className="max-w-xs"
