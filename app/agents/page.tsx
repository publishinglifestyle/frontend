...
export default function AgentsPage() {
          ...
          <Button
            ...
            onPress={async () => {
              ...
              setAgents((currentAgents) =>
                currentAgents.filter((w) => w.id !== item.id)
              );
              setIsLoading(false);
            }}
          >
  ...
  if (isLoading) {
    return (
      <div
        style={{ marginTop: "10%" }}
        className="text-center items-center align-center"
      >
            ...
            <div className="flex w-full flex-wrap md:flex-nowrap mb-6 md:mb-4 gap-4">
              ...
              >
                {agent_languages.map((agent_type) => (
                  <SelectItem key={agent_type.value}>
                    {agent_type.label}
                  </SelectItem>
                ))}
              </Select>
            ...
            <div className="flex w-full flex-wrap md:flex-nowrap mb-6 md:mb-0 gap-4">
              ...
              >
                {agent_levels.map((agent_level) => (
                  <SelectItem key={agent_level.value}>
                    {agent_level.label}
                  </SelectItem>
                ))}
              </Select>
              ...
              {agentType == "image" ? (
                ...
                >
                  {agent_models.map((agent_model) => (
                    <SelectItem key={agent_model.value}>
                      {agent_model.label}
                    </SelectItem>
                  ))}
                </Select>
              ...
              {n_buttons.map((btn) => (
                ...
                >
                  ...
                  >
                    ...
                    <p style={{ color: "#9353D3" }}>Buttons</p>
                  </CardHeader>
                </Card>
              ))}
            </div>
              ...
              >
                {columns.map((column) => (
                  <TableCell key={column.key}>
                    {renderCell(item, column.key as keyof Agent | "actions")}
                  </TableCell>
                ))}
              </TableRow>
            )}
